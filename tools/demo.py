...
def main():
    ...
    while True:
        ...
        cv2.imwrite('data/crowd_pose/test_out/' + '%d.jpg' % i, image)
        i += 1
...
def main1():
    ...
    # 图像文件
    imgPath = 'data/crowd_pose/test_img/'
    images = os.listdir(imgPath)
    print('测试图片数量为: ', len(images))

    for i in range(len(images)):
        image = cv2.imread(imgPath + images[i])
        a = datetime.datetime.now()
        ...
        image = cv2.putText(image, "{:.2f} ms / frame".format(inf_time), (40, 40),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
        # cv2.imshow('frame', image)
        cv2.imwrite('data/crowd_pose/test_img_out/' + '%d.jpg' % i, image)
        i += 1
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
    print('测试结束')
